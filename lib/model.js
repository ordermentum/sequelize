...
class Model {
  ...
  static _paranoidClause(model, options) {
    ...
    let deletedAtDefaultValue = deletedAtAttribute.hasOwnProperty(
      "defaultValue"
    )
    ...
    deletedAtObject[
      deletedAtAttribute.field || deletedAtCol
  ...
  static _conformOptions(options, self) {
    ...
    options.include = options.include.map(include =>
      this._conformInclude(include, self)
    );
  }
  ...
  static _transformStringAssociation(include, self) {
    if (self && typeof include === "string") {
      if (!self.associations.hasOwnProperty(include)) {
        throw new Error(
          'Association with alias "' + include + '" does not exists'
        );
      }
  ...
  static _conformInclude(include, self) {
    ...
    if (include instanceof Association) {
      ...
      if (include.association) {
        include.association = this._transformStringAssociation(
          include.association,
          self
        );
    ...
    for (const include of options.include) {
      ...
      if (
        ...
        if (include.duplicating) {
          ...
          include.subQuery =
            include.subQuery ||
            (include.hasParentRequired && include.hasRequired);
        }
      }
  ...
  static _validateIncludedElement(include, tableNames, options) {
    ...
    if (include.attributes && !options.raw) {
      ...
      if (include.attributes.length) {
        _.each(include.model.primaryKeys, (attr, key) => {
          ...
          if (
            !_.some(include.attributes, includeAttr => {
              if (attr.field !== key) {
                return (
    ...
    if (
      include.separate === true &&
      !(include.association instanceof HasMany)
    ) {
    ...
    if (
      ...
      !_.includes(options.attributes, association.source.primaryKeyAttribute)
    ) {
    ...
    if (include.hasOwnProperty("include")) {
      this._validateIncludedElements.call(
        include.model,
        include,
        tableNames,
        options
  ...
  static _getIncludedAssociation(targetModel, targetAlias) {
    ...
    if (associations.length === 0) {
      throw new sequelizeErrors.EagerLoadingError(
        `${targetModel.name} is not associated to ${this.name}!`
      );
      ...
      if (!association) {
        if (targetAlias) {
          throw new sequelizeErrors.EagerLoadingError(
            `${targetModel.name} is associated to ${
              this.name
            } using an alias. ` +
  ...
  static init(attributes, options) {
    ...
    if (this.options.timestamps) {
      if (this.options.createdAt !== false) {
        this._timestampAttributes.createdAt =
          ...
          Utils.underscoredIf("createdAt", this.options.underscored);
      }
      if (this.options.updatedAt !== false) {
        this._timestampAttributes.updatedAt =
          this.options.updatedAt ||
          Utils.underscoredIf("updatedAt", this.options.underscored);
      }
      if (this.options.paranoid && this.options.deletedAt !== false) {
        this._timestampAttributes.deletedAt =
          this.options.deletedAt ||
          Utils.underscoredIf("deletedAt", this.options.underscored);
      }
    }
    if (this.options.version) {
      this._versionAttribute =
        typeof this.options.version === "string"
    ...
    this._hasReadOnlyAttributes =
      this._readOnlyAttributes && this._readOnlyAttributes.length;
    this._isReadOnlyAttribute = _.memoize(
      key =>
        this._hasReadOnlyAttributes &&
        this._readOnlyAttributes.indexOf(key) !== -1
    );
  ...
  static refreshAttributes() {
    ...
    _.each(this.rawAttributes, (definition, name) => {
      ...
      if (definition.type instanceof DataTypes.BOOLEAN) {
        ...
        definition.type instanceof DataTypes.HSTORE ||
        DataTypes.ARRAY.is(definition.type, DataTypes.HSTORE)
      ) {
        this._hstoreAttributes.push(name);
      } else if (
        definition.type instanceof DataTypes.RANGE ||
        DataTypes.ARRAY.is(definition.type, DataTypes.RANGE)
      ) {
      ...
      if (definition.hasOwnProperty("defaultValue")) {
        this._defaultValues[name] = _.partial(
    ...
    this._isBooleanAttribute = _.memoize(
      key => this._booleanAttributes.indexOf(key) !== -1
    );

    this._hasDateAttributes = !!this._dateAttributes.length;
    this._isDateAttribute = _.memoize(
      key => this._dateAttributes.indexOf(key) !== -1
    );

    this._hasHstoreAttributes = !!this._hstoreAttributes.length;
    this._isHstoreAttribute = _.memoize(
      key => this._hstoreAttributes.indexOf(key) !== -1
    );

    this._hasRangeAttributes = !!this._rangeAttributes.length;
    this._isRangeAttribute = _.memoize(
      key => this._rangeAttributes.indexOf(key) !== -1
    );

    this._hasJsonAttributes = !!this._jsonAttributes.length;
    this._isJsonAttribute = _.memoize(
      key => this._jsonAttributes.indexOf(key) !== -1
    );

    this._hasVirtualAttributes = !!this._virtualAttributes.length;
    this._isVirtualAttribute = _.memoize(
      key => this._virtualAttributes.indexOf(key) !== -1
    );

    this._hasGeometryAttributes = !!this._geometryAttributes.length;
    this._isGeometryAttribute = _.memoize(
      key => this._geometryAttributes.indexOf(key) !== -1
    );
    ...
    this.prototype._isAttribute = _.memoize(
      key => this.prototype.attributes.indexOf(key) !== -1
    );
    ...
    if (this.primaryKeyAttribute) {
      this.primaryKeyField =
        this.rawAttributes[this.primaryKeyAttribute].field ||
        this.primaryKeyAttribute;
    }

    this._hasPrimaryKeys = this.primaryKeyAttributes.length > 0;
    this._isPrimaryKey = _.memoize(
      key => this.primaryKeyAttributes.indexOf(key) !== -1
    );
  }
  ...
  static sync(options) {
    ...
    return Promise.try(() => {
      ...
      .then(() =>
        this.QueryInterface.createTable(
      ...
      .then(() => {
        if (options.alter) {
          return Promise.all([
            ...
            this.QueryInterface.getForeignKeyReferencesForTable(
              this.getTableName(options)
            )
          ]).then(tableInfos => {
            ...
            _.each(columns, (columnDesc, columnName) => {
              ...
              if (!currentAttributes) {
                changes.push(() =>
                  this.QueryInterface.removeColumn(
                    this.getTableName(options),
                    columnName,
                    options
                  )
                ...
                if (currentAttributes.references) {
                  ...
                  _.each(foreignKeyReferences, foreignKeyReference => {
                    ...
                    if (
                      ...
                      (schema
                        ? foreignKeyReference.tableSchema === schema
                        : true) &&
                      foreignKeyReference.referencedTableName ===
                        references.model &&
                      foreignKeyReference.referencedColumnName ===
                        references.key &&
                      (schema
                        ? foreignKeyReference.referencedTableSchema === schema
                        : true) &&
                      !removedConstraints[constraintName]
                    ) {
                      // Remove constraint on foreign keys.
                      changes.push(() =>
                        this.QueryInterface.removeConstraint(
                          this.getTableName(options),
                          constraintName,
                          options
                        )
      ...
      .then(() =>
        this.QueryInterface.showIndex(this.getTableName(options), options)
      )
      .then(indexes => {
        // Assign an auto-generated name to indexes which are not named by the user
        this.options.indexes = this.QueryInterface.nameIndexes(
        ...
        indexes = _.filter(
          this.options.indexes,
          item1 => !_.some(indexes, item2 => item1.name === item2.name)
        ).sort((index1, index2) => {
  ...
  static addScope(name, scope, options) {
    ...
    if (
      (name === "defaultScope" || name in this.options.scopes) &&
      ...
      if (_.isPlainObject(option)) {
        if (option.method) {
          if (
            ...
            !!self.options.scopes[option.method[0]]
          ) {
            scopeName = option.method[0];
            scope = self.options.scopes[scopeName].apply(
              self,
        ...
        if (
          option === "defaultScope" &&
          _.isPlainObject(self.options.defaultScope)
        ) {
        ...
        _.assignWith(self._scope, scope, (objectValue, sourceValue, key) => {
          if (key === "where") {
            return Array.isArray(sourceValue)
              ? sourceValue
              : Object.assign(objectValue || {}, sourceValue);
          } else if (
            ["attributes", "include", "group"].indexOf(key) >= 0 &&
      ...
      .then(() => {
        if (options.include) {
          ...
          if (
            ...
            !options.raw &&
            this.primaryKeyAttribute &&
            options.attributes.indexOf(this.primaryKeyAttribute) === -1
          ) {
            ...
            if (
              ...
              options.attributes = [this.primaryKeyAttribute].concat(
                options.attributes
              );
            }
          }
  ...
  static warnOnInvalidOptions(options, validColumnNames) {
    ...
    const unrecognizedOptions = _.difference(
      Object.keys(options),
      validQueryKeywords
    );
    const unexpectedModelAttributes = _.intersection(
      unrecognizedOptions,
      validColumnNames
    );
    if (!options.where && unexpectedModelAttributes.length > 0) {
      Utils.warn(
        `Model attributes (${unexpectedModelAttributes.join(
          ", "
    ...
    if (options.limit === undefined) {
      ...
      if (
        ...
        !_.some(
          options.where,
          (value, key) =>
            (key === this.primaryKeyAttribute ||
              _.includes(uniqueSingleColumns, key)) &&
  ...
  static aggregate(attribute, aggregateFunction, options) {
    ...
    return this.QueryInterface.rawSelect(
      this.getTableName(options),
      options,
      aggregateFunction,
      this
    ...
    return this.sequelize
      ...
      .then(instance => {
        ...
        return this.create(values, options)
          .then(instance => {
            if (
              instance.get(this.primaryKeyAttribute, { raw: true }) === null
            ) {
          ...
          .catch(this.sequelize.UniqueConstraintError, err => {
            ...
            const flattenedWhereKeys = _.map(_.keys(flattenedWhere), name =>
              _.last(_.split(name, "."))
            );
            ...
            const defaultFields =
              options.defaults &&
              Object.keys(options.defaults).map(
                name => this.rawAttributes[name].field || name
              );

            if (defaultFields) {
              if (
                !_.intersection(Object.keys(err.fields), whereFields).length &&
                _.intersection(Object.keys(err.fields), defaultFields).length
              ) {
                throw err;
              }
            }
              ...
              _.each(err.fields, (value, key) => {
                ...
                if (value.toString() !== options.where[name].toString()) {
                  throw new Error(
                    `${
                      this.name
                    }#findOrCreate: value used for ${name} was not equal for both the find and the create calls, '${
                      options.where[name]
                    }' vs '${value}'`
                  );
                }
  ...
  static findCreateFind(options) {
    ...
    return this.findOne(options).then(result => {
      ...
      return this.create(values, options)
        ...
        .catch(this.sequelize.UniqueConstraintError, () =>
          this.findOne(options).then(result => [result, false])
        );
    });
  }
      ...
      return Promise.try(() => {
        ...
        .spread((created, primaryKey) => {
          if (options.returning === true && primaryKey) {
            return this.findByPk(primaryKey, options).then(record => [
              record,
              created
            ]);
          }
  ...
  static bulkCreate(records, options) {
    ...
    if (options.ignoreDuplicates && dialect === "mssql") {
      return Promise.reject(
        new Error(dialect + " does not support the 'ignoreDuplicates' option.")
      );
    }
    if (options.updateOnDuplicate && dialect !== "mysql") {
      return Promise.reject(
        new Error(dialect + " does not support the 'updateOnDuplicate' option.")
      );
    }

    if (options.updateOnDuplicate) {
      // By default, all attributes except 'createdAt' can be updated
      let updatableAttributes = _.pull(
        Object.keys(this.tableAttributes),
        "createdAt"
      );
      if (
        _.isArray(options.updateOnDuplicate) &&
        !_.isEmpty(options.updateOnDuplicate)
      ) {
        updatableAttributes = _.intersection(
          updatableAttributes,
    ...
    let instances = records.map(values =>
      this.build(values, { isNewRecord: true })
    );
      ...
      .then(() => {
        for (const instance of instances) {
          ...
          if (createdAtAttr && !values[createdAtAttr]) {
            ...
            options.fields.indexOf(createdAtAttr) === -1 &&
              options.fields.push(createdAtAttr);
          }
          if (updatedAtAttr && !values[updatedAtAttr]) {
            values[updatedAtAttr] = now;
            options.fields.indexOf(updatedAtAttr) === -1 &&
              options.fields.push(updatedAtAttr);
          }

          instance.dataValues = Utils.mapValueFieldNames(
            values,
            options.fields,
            this
          );
        ...
        if (options.individualHooks) {
          ...
          if (options.updateOnDuplicate) {
            options.updateOnDuplicate = options.updateOnDuplicate.map(
          ...
          return this.QueryInterface.bulkInsert(
            ...
            if (Array.isArray(results)) {
              results.forEach((result, i) => {
                if (
                  instances[i] &&
                  !instances[i].get(this.primaryKeyAttribute)
                ) {
                  instances[i] &&
                    instances[i].set(
                      this.primaryKeyAttribute,
                      result[this.primaryKeyField],
                      { raw: true }
                    );
                }
              });
        ...
        instances.forEach(instance => {
          for (const attr in this.rawAttributes) {
            if (
              ...
              instance.dataValues[this.rawAttributes[attr].field] !==
                undefined &&
              this.rawAttributes[attr].field !== attr
            ) {
              instance.set(
    ...
    return Promise.try(() => {
      ...
      .then(() => {
        ...
        if (options.individualHooks) {
          return this.findAll({
            ...
            .map(instance =>
              this.runHooks("beforeDestroy", instance, options).then(
                () => instance
              )
            )
            .then(_instances => {
      ...
      .then(() => {
        ...
        if (this._timestampAttributes.deletedAt && !options.force) {
          ...
          const attrValueHash = {};
          const deletedAtAttribute = this.rawAttributes[
            this._timestampAttributes.deletedAt
          ];
          const field = this.rawAttributes[this._timestampAttributes.deletedAt]
            .field;
          const where = {};

          where[field] = deletedAtAttribute.hasOwnProperty("defaultValue")
            ? deletedAtAttribute.defaultValue
            : null;
          ...
          return this.QueryInterface.bulkDelete(
            this.getTableName(options),
            options.where,
            options,
            this
      ...
      .tap(() => {
        ...
        if (options.individualHooks) {
          return Promise.map(instances, instance =>
            this.runHooks("afterDestroy", instance, options)
          );
        }
      })
      .tap(() => {
  ...
  static restore(options) {
    ...
    return Promise.try(() => {
      ...
      .then(() => {
        ...
        if (options.individualHooks) {
          return this.findAll({
            ...
            .map(instance =>
              this.runHooks("beforeRestore", instance, options).then(
                () => instance
              )
            )
            .then(_instances => {
      ...
      .then(() => {
        ...
        const deletedAtDefaultValue = deletedAtAttribute.hasOwnProperty(
          "defaultValue"
        )
          ? deletedAtAttribute.defaultValue
          : null;
      ...
      .tap(() => {
        ...
        if (options.individualHooks) {
          return Promise.map(instances, instance =>
            this.runHooks("afterRestore", instance, options)
          );
        }
      })
      .tap(() => {
  ...
  static update(values, options) {
    ...
    if (this._timestampAttributes.updatedAt && !options.silent) {
      values[this._timestampAttributes.updatedAt] =
        this._getDefaultTimestamp(this._timestampAttributes.updatedAt) ||
        Utils.now(this.sequelize.options.dialect);
    }
  ...
  static describe(schema, options) {
    return this.QueryInterface.describeTable(
      this.tableName,
      _.assign({ schema: schema || this._schema || undefined }, options)
    );
  }

  static _getDefaultTimestamp(attr) {
    if (!!this.rawAttributes[attr] && !!this.rawAttributes[attr].defaultValue) {
      return Utils.toDefaultValue(
        this.rawAttributes[attr].defaultValue,
        this.sequelize.options.dialect
      );
    }
    return undefined;
  ...
  static _injectScope(options) {
    ...
    if (scope.include) {
      ...
      for (const scopeInclude of scope.include.reverse()) {
        if (
          scopeInclude.all ||
          !options.include.some(item => {
            const isSameModel =
              item.model && item.model.name === scopeInclude.model.name;
            ...
            if (scopeInclude.as) {
              ...
              const association =
                scopeInclude.association ||
                this.getAssociationForAlias(
                  scopeInclude.model,
                  scopeInclude.as
                );
  ...
  static increment(fields, options) {
    ...
    if (!options.silent && updatedAtAttr && !values[updatedAtAttr]) {
      options.attributes[updatedAtAttribute.field || updatedAtAttr] =
        this._getDefaultTimestamp(updatedAtAttr) ||
        Utils.now(this.sequelize.options.dialect);
    }
    if (versionAttr) {
  ...
  */
    ...
    if (options.isNewRecord) {
      ...
      if (this.constructor._hasDefaultValues) {
        defaults = _.mapValues(this.constructor._defaultValues, valueFn => {
          const value = valueFn();
          return value && value instanceof Utils.SequelizeMethod
            ? value
            : _.cloneDeep(value);
        });
      }
      ...
      if (
        ...
        !defaults.hasOwnProperty(this.constructor.primaryKeyAttribute)
      ) {
   ...
   */
  where(checkVersion) {
    const where = this.constructor.primaryKeyAttributes.reduce(
      (result, attribute) => {
  ...
  get(key, options) {
    ...
    if (key) {
      ...
      if (
        ...
        this._options.includeNames.indexOf(key) !== -1
      ) {
  ...
  set(key, value, options) {
    ...
    if (typeof key === "object" && key !== null) {
      ...
      if (!options.raw && this._customSetters[key]) {
        ...
        if (
          (!Utils.isPrimitive(newValue) && newValue !== null) ||
        ...
        if (
          ...
          if (!options.raw) {
            ...
            if (!this._isAttribute(key)) {
              if (
                key.indexOf(".") > -1 &&
                this.constructor._isJsonAttribute(key.split(".")[0])
              ) {
            ...
            if (
              ...
              this.constructor._isReadOnlyAttribute(key)
            ) {
              return this;
            }
          }
          ...
          if (
            ...
            this.constructor._dataTypeSanitizers.hasOwnProperty(key)
          ) {
            value = this.constructor._dataTypeSanitizers[key].call(
              this,
              value,
              options
            );
          ...
          if (
            ...
            (value instanceof Utils.SequelizeMethod ||
              ...
              (!(value instanceof Utils.SequelizeMethod) &&
                ...
                this.constructor._dataTypeChanges[key].call(
                  this,
                  value,
                  originalValue,
                  options
  ...
  previous(key) {
    ...
    return _.pickBy(this._previousDataValues, (value, key) =>
      this.changed(key)
    );
  }
  ...
  _setInclude(key, value, options) {
    ...
    if (
      include.originalAttributes === undefined ||
      include.originalAttributes.length
    ) {
      if (association.isSingleAssociation) {
        ...
        isEmpty =
          (value && value[primaryKeyAttribute] === null) || value === null;
        value = !isEmpty && _.assign(value, association.scope);
        this[accessor] = this.dataValues[accessor] = isEmpty
          ? null
  ...
  save(options) {
    if (arguments.length > 1) {
      throw new Error(
        "The second argument was removed in favor of the options object."
      );
    }
    ...
    const primaryKeyAttribute =
      primaryKeyName && this.constructor.rawAttributes[primaryKeyName];
    ...
    if (
      versionAttr &&
      options.fields.length >= 1 &&
      options.fields.indexOf(versionAttr) === -1
    ) {
    ...
    if (
      options.silent === true &&
      !(this.isNewRecord && this.get(updatedAtAttr, { raw: true }))
    ) {
    ...
    if (this.isNewRecord === true) {
      ...
      if (
        ...
        options.fields.indexOf(primaryKeyName) < 0
      ) {
    ...
    if (
      ...
      this.dataValues[updatedAtAttr] =
        this.constructor._getDefaultTimestamp(updatedAtAttr) || now;
    }

    if (this.isNewRecord && createdAtAttr && !this.dataValues[createdAtAttr]) {
      this.dataValues[createdAtAttr] =
        this.constructor._getDefaultTimestamp(createdAtAttr) || now;
    }
    ...
    return Promise.try(() => {
      ...
      .then(() => {
        ...
        if (options.hooks) {
          ...
          return this.constructor
            .runHooks("before" + hook, this, options)
            .then(() => {
              if (options.defaultFields && !this.isNewRecord) {
                afterHookValues = _.pick(
                  this.dataValues,
                  _.difference(this.changed(), ignoreChanged)
                );
      ...
      .then(() => {
        ...
        return Promise.map(
          this._options.include.filter(
            include => include.association instanceof BelongsTo
          ),
          include => {
            ...
            return instance.save(includeOptions).then(() =>
              this[include.association.accessors.set](instance, {
                save: false,
                logging: options.logging
              })
            );
      ...
      .then(() => {
        const realFields = options.fields.filter(
          field => !this.constructor._isVirtualAttribute(field)
        );
        if (!realFields.length) return this;
        if (!this.changed() && !this.isNewRecord) return this;

        const versionFieldName =
          _.get(this.constructor.rawAttributes[versionAttr], "field") ||
          versionAttr;
        let values = Utils.mapValueFieldNames(
          this.dataValues,
          options.fields,
          this.constructor
        );
        ...
        if (this.isNewRecord) {
          ...
          where = Utils.mapValueFieldNames(
            where,
            Object.keys(where),
            this.constructor
          );
        ...
        return this.constructor.QueryInterface[query]
          .apply(this.constructor.QueryInterface, args)
          .then(results => {
            ...
            for (const attr of Object.keys(this.constructor.rawAttributes)) {
              if (
                ...
                values[this.constructor.rawAttributes[attr].field] !==
                  undefined &&
              ...
              include => {
                ...
                return Promise.map(instances, instance => {
                  if (include.association instanceof BelongsToMany) {
                    return instance.save(includeOptions).then(() => {
                      const values = {};
                      values[include.association.foreignKey] = this.get(
                        this.constructor.primaryKeyAttribute,
                        { raw: true }
                      );
                      values[include.association.otherKey] = instance.get(
                      ...
                      return include.association.throughModel.create(
                        values,
                        includeOptions
                      );
                    });
  ...
  update(values, options) {
    ...
    const sideEffects = _.without.apply(
      this,
      [this.changed() || []].concat(changedBefore)
    );
    ...
    return Promise.try(() => {
      ...
      .then(() => {
        ...
        if (
          ...
          const field =
            attribute.field || this.constructor._timestampAttributes.deletedAt;
          const values = {};

          values[field] = new Date();
          where[field] = attribute.hasOwnProperty("defaultValue")
          ...
          return this.constructor.QueryInterface.update(
            ...
            values,
            where,
            _.defaults({ hooks: false, model: this.constructor }, options)
          ).then(results => {
            const rowsUpdated = results[1];
          ...
          return this.constructor.QueryInterface.delete(
            this,
            this.constructor.getTableName(options),
            where,
            _.assign({ type: QueryTypes.DELETE, limit: null }, options)
          );
        }
      })
      .tap(() => {
  ...
  isSoftDeleted() {
    ...
    const deletedAtAttribute = this.constructor.rawAttributes[
      this.constructor._timestampAttributes.deletedAt
    ];
    const defaultValue = deletedAtAttribute.hasOwnProperty("defaultValue")
      ? deletedAtAttribute.defaultValue
      : null;
  ...
  restore(options) {
    ...
    return Promise.try(() => {
      ...
      .then(() => {
        ...
        return this.save(
          _.extend({}, options, { hooks: false, omitNull: false })
        );
      })
      .tap(() => {
        // Run after hook
  ...
  equals(other) {
    ...
    return _.every(
      this.constructor.primaryKeyAttributes,
      attribute =>
        this.get(attribute, { raw: true }) ===
        other.get(attribute, { raw: true })
    );
  }
...
Model.findByPrimary = function() {
  Utils.deprecate(
    "Model.findByPrimary has been deprecated, please use Model.findByPk instead"
  );
  return Model.findByPk.apply(this, arguments);
};
Model.findById = function() {
  Utils.deprecate(
    "Model.findById has been deprecated, please use Model.findByPk instead"
  );
  return Model.findByPk.apply(this, arguments);
};
Model.find = function() {
...
Model.findAndCount = function() {
  Utils.deprecate(
    "Model.findAndCount has been deprecated, please use Model.findAndCountAll instead"
  );
  return Model.findAndCountAll.apply(this, arguments);
};
Model.findOrInitialize = function() {
  Utils.deprecate(
    "Model.findOrInitialize has been deprecated, please use Model.findOrBuild instead"
  );
  return Model.findOrBuild.apply(this, arguments);
};
Model.insertOrUpdate = function() {
  Utils.deprecate(
    "Model.insertOrUpdate has been deprecated, please use Model.upsert instead"
  );
  return Model.upsert.apply(this, arguments);
};
Model.all = function() {
  Utils.deprecate(
    "Model.all has been deprecated, please use Model.findAll instead"
  );
  return Model.findAll.apply(this, arguments);
};
Model.prototype.updateAttributes = function() {
  Utils.deprecate(
    "Instance.updateAttributes has been deprecated, please use Instance.update instead"
  );
