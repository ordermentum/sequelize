...
const QueryGenerator = {
  ...
  */
  ...
  whereItemsQuery(where, options, binding) {
    ...
    if (keys.length > 10) {
      console.error('Large Where Query', JSON.stringify(keys), JSON.stringify(where));
    }
    ...
    if (_.isPlainObject(where)) {
      keys.forEach(prop => {
        ...
        try {
          ...
          console.error('Failed to generate where item query', prop, util.inspect(item), e, util.inspect(where));
          throw e;
        }
      });
    } else {
